...
import com.go2plus.core.promotion.service.SearchResultPagePromotionService;
...
public class SearchResultPageController extends BaseController {
  ...
  private final static Logger              log      = LoggerFactory.getLogger(SearchResultPageController.class);

  @Autowired
  private SearchResultPagePromotionService searchResultCenterService;
  ...
  public ModelAndView searchResult(HttpServletRequest request, HttpServletResponse response) {
    ...
    try {
      ...
      boolean isShow = true;
      // 获取商家信息
      Supplier supplier = searchResultCenterService.getByUserId(userId);
      box.setAttribute("supplier", supplier);
      if (supplier.getMarketId() != 5) {
        box.setAttribute("msg", "对不起, 搜索结果广告目前只面向国际商贸城高级认证厂家开放！");
        return createModelAndView("core/promotion/searchPage.jsp", box);
      } else if (supplier.getCertifiedType() != 2) {
        box.setAttribute("msg", "对不起, 搜索结果广告目前只面向国际商贸城高级认证厂家开放，请联系客服QQ 4006611603 开通高级认证！");
        return createModelAndView("core/promotion/searchPage.jsp", box);
      }
      ...
      if (searchResultCenterService.checkSPRule(startTime, userId) > 0) {
        isShow = false;
        box.setAttribute("reason", "恭喜你，您已订购成功同期开始的搜索结果广告位或搜索结果广告位<br>请到“<a href=\"/manage/promotion/myfps\">我订购的广告</a>”中查看！");
      }
      if (isShow) {
        Integer totalFee = 300;
        double discount = 1;
        int num = 29;
        List<AdPosition> map = searchResultCenterService.getSRsBy("3");
  ...
  public ModelAndView toSearchPage(String posX, String startTime, Integer totalFee, Double discount, String hash,
    ...
    try {
      ...
      if (validatorService.isOnBlackList(userId)) {
        ...
        return createModelAndView("core/promotion/order_search.jsp", box);
      }
      ...
      if (validatorService.disableFunc(userId)) {
        box.setAttribute("msg", "由于涉嫌违规，您订购搜索结果广告的权利已被限制，详情请与客服联系");
        return createModelAndView("core/promotion/order_search.jsp", box);
      }
      ...
      Thread.sleep(1);
      if (searchResultCenterService.checkSPRule(promotion.getStartTime(), userId) > 0) {
        box.setAttribute("msg", "对不起，您本期已经订购了搜索结果广告，把机会留点给别的商家吧！");
        return createModelAndView("core/promotion/order_search.jsp", box);
      }
      String[] operators = { "-", "+" };
      int[] arr = new int[2];
      ...
      int num = 0;
      // 上周二搜索结果|搜索结果
      int num1 = searchResultCenterService.checkSPRule(getDateByTue(-1), userId);
      if (num1 > 0) {
        num += num1;
      }
      // 本周二搜索结果|搜索结果
      int num2 = searchResultCenterService.checkSPRule(getDateByTue(0), userId);
      if (num2 > 0) {
        num += num2;
      }
      // 下周二搜索结果|搜索结果
      int num3 = searchResultCenterService.checkSPRule(getDateByTue(1), userId);
      if (num3 > 0) {
        num += num3;
      }
  ...
  @ResponseBody
  public JSONObject search_Page(String hash, String refer, int x, int y, String operator, int result, String posX, String startTime,
    ...
    try {
      ...
      if (useRedis) {
        supplier = searchResultCenterService.getByUserIdFromRedis(userId);
      } else {
        supplier = searchResultCenterService.getByUserId(userId);
      }
      ...
      // 权限认证检验
      if (supplier.getMarketId() != 5) {
        object.put("msg", "对不起, 搜索结果广告目前只面向国际商贸城高级认证厂家开放！");
        return object;

      } else if (supplier.getCertifiedType() != 2) {
        object.put("msg", "对不起, 搜索结果广告目前只面向国际商贸城高级认证厂家开放，请联系客服QQ 4006611603 开通高级认证！");
        return object;
      }

      if (validatorService.disableFunc(userId)) {
        object.put("msg", "由于涉嫌违规，您订购搜索结果广告的权利已被限制，详情请与客服联系");
        return object;
      }
      ...
      Thread.sleep(1);
      if (searchResultCenterService.checkSPRule(promotion.getStartTime(), userId) > 0) {
        object.put("reason", "对不起，您本期已经订购了搜索结果广告，把机会留点给别的商家吧！");
        return object;
      }

      if (StringUtils.isEmpty(refer)) {
        refer = "/promotion/searchResult";
      }
      ...
      if (validatorService.isAvaiableFP(posX, promotion.getStartTime())) {
        if (searchResultCenterService.addFP(promotion) && searchResultCenterService.delAD(posX, startTime, "3")) {
          object.put("success", true);
          object.put("msg", "恭喜您，搜索结果广告位" + posX + "（开始时间：" + startTime + "）订购成功！");
          return object;
          // // 复核
          // if(searchResultCenterService.getFP(promotion.getStartTime(), userId, posX)){
          // }else{
