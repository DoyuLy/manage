		...
		if ( elements != null && elements.size() > 0 ) {
			...
			while ( iterator.hasNext() ) {
				...
				if ( enable == null || enable.equals("true") ) {
					...
					if( global.get("needSchema") !=null ){
					  maker.setNeedSchema(Boolean.valueOf(global.get("needSchema").toString()));
					}
					if( global.get("entityPackageName") !=null ){
            maker.setEntityPackageName(global.get("entityPackageName").toString());
          }
					else{
					  maker.setEntityPackageName("entity");
					}
					if( global.get("daoPackageName") !=null ){
            maker.setDaoPackageName(global.get("daoPackageName").toString());
          }
          else{
            maker.setEntityPackageName("repository");
          }
