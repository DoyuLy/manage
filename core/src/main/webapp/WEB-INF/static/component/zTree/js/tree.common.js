...
//                zTree.selectNode(newNode);
    ...
    initSelectTree : function(config) {
        ...
        function showMenu() {
            ...
            if(width<200) width=width+80;
            $treeContent.css({left: nameOffset.left + "px", top: nameOffset.top + $name.outerHeight() + "px",width: width+"px"}).slideDown("fast");
        ...
        config.select.btn.click(function () {
        	if(config.select.condition){
        		if (!config.select.condition()) return false;
        	}
        ...
        var initTree = function() {
            ...
            if(autocomplateEnable) {
                ...
                if(config.data){
	                $.each(config.data,function(index, item){
	                	config.autocomplete.source += "&" +index+"="+item;
	                });
                }
                config.treeId = treeSelect;
                $(config.autocomplete.input).data("ztree",zTree);
                $.app.initAutocomplete(config.autocomplete);
            }
        };
